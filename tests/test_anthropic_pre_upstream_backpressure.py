...
class _DummyAnthropicHandler(AnthropicHandlerMixin):
    ...
    async def _retry_request(
        ...
        *,
        ...
        # ``_retry_request`` signature; the engine flip added
        # ``override_outbound_bytes``. The dummy stub doesn't need to use
        # them — just accept them so existing tests don't break with
        # TypeError on the new call sites.
        del original_body_bytes, body_mutated, mutation_reasons
        del request_id, forwarder_name, path_for_log, override_outbound_bytes
